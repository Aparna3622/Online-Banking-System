...
def withdraw(ls, acc_list):
    ...
    def witClicked(current_balance, ls):
        ...
        tk.Label(witTop, text=bal, font=("ubuntu", 12), bg="#black").grid(column=0, row=3, columnspan=2, padx=(0, 10),
                                                                          pady=(10, 10))
    ...
    tk.Label(witTop, text=cBalance, font=("ubuntu", 12), fg="#4267B2").grid(column=0, row=0, pady=(10, 10),
                                                                            padx=(10, 10))
    tk.Label(witTop, text="Withdraw Amount ", font=("ubuntu", 12), fg="#4267B2").grid(column=0, row=1, pady=(10, 10),
                                                                                      padx=(10, 10))
...
def deposit(ls, acc_list):
    ...
    def confirmClicked(current_balance, ls):
        ...
        ls[3] = str(current_balance)
        bal = "Your current balance: " + ls[3]
        tk.Label(depositTop, text=bal, font=("ubuntu", 12), fg="#4267B2").grid(column=0, row=3, columnspan=2,
    ...
    cBalance = 'Current balance: ' + ls[3]
    tk.Label(depositTop, text=cBalance, font=("ubuntu", 12), fg="#4267B2").grid(column=0, row=0, pady=(10, 10),
                                                                                padx=(10, 10))
    tk.Label(depositTop, text="Deposit Amount ", font=("ubuntu", 12), fg="#4267B2").grid(column=0, row=1, pady=(10, 10),
                                                                                         padx=(10, 10))
...
def pin_change(ls, acc_list):
    ...
    ls: Current account info [id, name, password, balance]
    acc_list: List of all accounts
    """

    def pin_button_clicked(): 
        try:
            old_pin = OldPinEntry.get()
            new_pin = NewPinEntry.get()
            confirm_pin = ConformPinEntry.get()

            # Validate input formats
            if not (old_pin.isdigit() and new_pin.isdigit() and confirm_pin.isdigit()):
    ...
    pinTop.title("Change PIN")

    # Create and layout widgets
    tk.Label(pinTop, text="Change PIN", font=("ubuntu", 14), fg="#4267B2").grid(
        column=0, row=0, columnspan=2, pady=(10, 10), padx=(10, 10))

    tk.Label(pinTop, text="Current PIN:", font=("ubuntu", 12), fg="#4267B2").grid(
        column=0, row=1, pady=(10, 10), padx=(10, 10))
    OldPinEntry = tk.Entry(pinTop, font=("ubuntu", 12), fg="red", width=8, show="*")
    OldPinEntry.grid(column=1, row=1, padx=(10, 10), pady=(10, 10))

    tk.Label(pinTop, text="New PIN:", font=("ubuntu", 12), fg="#4267B2").grid(
        column=0, row=2, pady=(10, 10), padx=(10, 10))
    NewPinEntry = tk.Entry(pinTop, font=("ubuntu", 12), fg="red", width=8, show="*")
    NewPinEntry.grid(column=1, row=2, padx=(10, 10), pady=(10, 10))

    tk.Label(pinTop, text="Confirm PIN:", font=("ubuntu", 12), fg="#4267B2").grid(
        column=0, row=3, pady=(10, 10), padx=(10, 10))
    ConformPinEntry = tk.Entry(pinTop, font=("ubuntu", 12), fg="red", width=8, show="*")
    ...
    tk.Button(pinTop, text="Change PIN", font=("ubuntu", 12), fg="white", bg="#4267B2",
              command=pin_button_clicked).grid(column=0, row=4, columnspan=2, pady=(10, 10))
...
def show_history(ls):
    ...
    top_line = '\nID\t   ' + 'Type'.center(len('change_password') + 4) + 'Date and Time'.center(
        40 + 7) + 'before'.center(
        15) + 'after'.center(20)
    # print(top_line)
    ...
    for line in id_list:
        ...
        hline = ('{0}\t{1}\t{2}{3: ^10} {4: ^10}'.format(line[0], line[1].center(len('change_password')),
                                                         date.center(len(date)), line[7], line[8]))
        depline = depline + "\n\n " + hline
...
def mainWindow():
    def loginClicked():
        ...
        label = tk.Label(loginWindow, text="LPU ATM Machine", font=("ubuntu", 33), bg="#4267B2", fg="#FFFFFF",
                         width=100)
        label.place(relx=0.5, rely=0.05, anchor="center")
        backLogout = "Back"
        LoginButton = tk.Button(loginWindow, text=backLogout, font=("ubuntu", 33), bg="#4267B2", width=100,
                                fg="#FFFFFF",
    ...
    def createAccountClicked():
        ...
        label = tk.Label(CreatedWindow, text="LPU ATM Machine", font=("ubuntu", 33), bg="#4267B2", fg="#FFFFFF",
                         width=100)
        label.place(relx=0.5, rely=0.05, anchor="center")
